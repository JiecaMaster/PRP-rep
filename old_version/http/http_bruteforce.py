...
for webstream, data in webstream_data.items():
    if first:
        ...
        print(chain_with_history.invoke(
            {"question": "我将向你提供一份http连接的记录，其中包含源IP，目标IP，uri，referer，请求方法5个字段的信息。对于每条记录，请你进行以下操作：1.判断请求方法字段是否为“POST”，http层面的登录请求为保护账号密码信息通常使用post方法发送信息。如果不包含，则这条记录无需被统计；如果包含，则进行接下来的操作。2.检查这条连接的源IP与目标IP，uri，referer构成的四元组是否曾经出现过：如果没有出现过，则在统计结果中添加这个四元组，并将这个四元组的计数增加到1；如果已经出现过了，则在统计结果中直接将这个四元组的计数加1。注意按照记录序号由小到大进行统计，不要重复统计，也不要遗漏。http连接记录如下：" + data},
            config={"configurable": {"session_id": "http_bruteforce"}}
        ))
        first = False
    else:
        # 对于其他webstream的处理
...
print(chain_with_history.invoke(
    {"question":"请根据统计的结果，将所有计数超过10的四元组中的源IP与目标IP输出.如果没有计数超过10或根本没有计数存在，则输出：http连接上未检测到暴力破解攻击。"},
    config={"configurable":{"session_id":"http_bruteforce"}}
))
