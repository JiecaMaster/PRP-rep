...
# 假设FTP文件位于同一目录下
file_path = 'conn_ftp_bruteforce.txt'
...
for webstream, data in webstream_data.items():
    if first:
        ...
        print(chain_with_history.invoke(
            {"question": "我将向你提供一份ftp连接的记录，其中包含源IP，目标IP与历史信息3个字段的信息。对于每条记录，请你进行以下操作：1.判断历史信息字段是否包含小写r这一字母，当历史信息包含这一字母时，通常表示此次的连接尝试被接收方中止。如果不包含，则这条记录无需被统计；如果包含，则进行接下来的操作。2.检查这条连接的源IP与目标IP对是否曾经出现过：如果这对IP没有出现过，则在统计结果中添加这对IP，并将这对IP的计数增加到1；如果已经出现过了，则在统计结果中直接将这对IP的计数加1。注意按照记录序号由小到大进行统计，不要重复统计，也不要遗漏。ftp连接记录如下：" + data},
            config={"configurable": {"session_id": "ftp_bruteforce"}}
        ))
        first = False
    else:
        # 对于其他webstream的处理
        print(chain_with_history.invoke(
            {"question": "在前面结果的基础上，额外统计以下记录，注意按照记录序号由小到大进行逐条统计，不要重复统计，也不要遗漏：" + data},
            config={"configurable": {"session_id": "ftp_bruteforce"}}
        ))

print(chain_with_history.invoke(
    {"question":"请根据统计的结果，将所有计数超过20的IP对输出.如果没有计数超过20或根本没有计数存在，则输出：ftp连接未受到暴力破解攻击。"},
    config={"configurable":{"session_id":"ftp_bruteforce"}}
))