...
def choose_fields_and_filter(input_file_path, output_file_path):
    # 要保留的字段名
    fields_to_keep = [ 'id.orig_h', 'id.resp_h','service','conn_state','history']
...
def process_log_file(input_filename, output_filename):
    ...
    for line_number, line in enumerate(lines[1:], start=1):  # 从第二行开始处理，行数从1开始
        ...
        processed_line = f'记录{line_number}: [' + ', '.join(processed_fields) + '];'
...
def main():
    # 设置输入和输出文件的路径
    input_file_path = 'conn_http_select.txt'
    intermediate_file_path = 'conn_http_select_mid.txt'
    output_file_path = 'conn_dos.txt'
